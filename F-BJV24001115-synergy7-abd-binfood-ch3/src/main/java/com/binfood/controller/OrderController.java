...
public class OrderController {
    ...
    public void editOrder(boolean bypass, String choice, String qty) {
        while (true) {
            SystemUtils.clearConsole();
            displayOrder();
            System.out.println("Pilih item untuk diedit (0 untuk kembali):");
            int itemChoice = bypass ? Integer.parseInt(choice) : scanner.nextInt();
            if (itemChoice == 0) {
                return;
            }
            ...
            if (itemChoice > 0 && itemChoice <= itemNames.size()) {
                ...
                int editChoice = bypass ? Integer.parseInt(choice) : scanner.nextInt();
                if (editChoice == 1) {
                    System.out.println("Masukkan jumlah baru:");
                    int newQuantity = bypass ? Integer.parseInt(qty) : scanner.nextInt();
